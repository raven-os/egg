...
from tests.install_queue import captured_output
...
class TestBasicEvent(unittest.TestCase):
    ...
    def test_basic_event(self):
        event = BasicEvent(BasicTestClass.printTest.__name__, value=1)
        InstallQueue().add(event)
        with captured_output() as (out, err):
            InstallQueue().execAll()
        output = out.getvalue().strip()
        self.assertEqual(output, 'method_name: printTest, value: 1')
